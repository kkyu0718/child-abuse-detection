...
def main():
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            new_path = '/content/gdrive/Shareddrives/2021청년인재_고려대과정_10조/BiConvLSTM_Violence_Detection_Spatiotemporal_Encoder/'
            f_name = os.path.basename(args.resume)
            graph = np.load(new_path  +'acc_graph/'+args.resume.split('/')[-2]+'/'+args.resume.split('/')[-2]+'acc_loss_prec_'+re.findall(r'_t(.+).tar',f_name)[0]+'.npy',allow_pickle=True)
    ...
    run_training(train_loader, val_loader, model, criterion, optimizer, best_prec, nowDatetime, acc, loss, prec, f1score, graph_path, args.ID)
...
def run_training(train_loader, val_loader, model, criterion, optimizer, best_prec, starttime, accuray_list,loss_list, precision_list, f1score_list, graph_path, ID):
    ...
    while True:
        ...
        np.save(graph_path + 'acc_loss_prec_' + starttime ,[accuray_list,loss_list,precision_list, f1score_list])
        print(graph_path + 'acc_loss_prec_' + starttime)
        ...
        save_checkpoint({
            ...
            }, is_best, args.arch + "_" + args.data_name + "_fold_" + str(args.kfold), starttime=starttime)
        epoch += 1
